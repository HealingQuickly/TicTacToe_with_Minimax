...
void Board::print() const {
	...
	// y and x represent the position of one whole big block
	for (int y = 0; y < _size; y++) {
		// Add line
		addHorizontalLine(text);
		// Loop through glyph vertically
		for (int gy = 0; gy < 3; gy++) {
			...
			for (int x = 0; x < _size; x++) {
				// Loop through glyph horizontally and append the tile
				text += "|";
				for (int gx = 0; gx < 3; gx++) {
					// getVal returns any of (0, 1, 2) which determines (NO, X, O)
					text += GLYPHS[getVal(x, _size - y - 1)][gy][gx]; // the single tile on [gx, gy] spot
				}
			}
			text += "|";
